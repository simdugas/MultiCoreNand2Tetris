...
public class CPUEmulatorMain
{
  ...
  public static void main(String[] args) {
        if ( args.length > 2 )
            System.err.println("Usage: java CPUEmulatorMain [num cores] [script name]");
        else {
            int numCores = 1;
            if( args.length > 0 ) {
            ...
            if (args.length < 2) {
                ...
                CPUEmulatorGUI simulatorGUI;
                if ( numCores > 1 ) {
                    simulatorGUI = new MultiCoreCPUEmulatorComponent(numCores);
                } else {
                    simulatorGUI = new SingleCoreCPUEmulatorComponent();
                }
                ...
                new HackController(new CPUEmulator( numCores ), args[0]);
            }
        }
    }
}
